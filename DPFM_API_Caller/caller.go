...
import (
	...
	"data-platform-api-planned-freight-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) headerDelete(
	...
	header.PlannedFreight = input.Header.PlannedFreight
	header.IsMarkedForDeletion = input.Header.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "PlannedFreightHeader", "runtime_session_id": sessionID})
	if err != nil {
